...
interface GradientCreatorProps {
  onCustomGradient: (gradient: any) => void;
...
export default function GradientCreator({ onCustomGradient, isOpen }: GradientCreatorProps) {
      ...
      background: {
        ...
        gradient: {
          ...
          colorStops: [
            ...
            { offset: 1, color: colorStops[colorStops.length - 1]?.color + '10' || '#FFE5B410' }
          ]
        }
      },
      cornersSquare: {
            ...
            {colorStops.map((stop, index) => (
              <div key={index} class="flex items-center gap-3">
                <input
                  type="color"
                  value={stop.color}
                  onInput={(e) => updateColorStop(index, 'color', (e.target as HTMLInputElement).value)}
                ...
                <input
                  ...
                  step="0.01"
                  value={stop.offset}
                  onInput={(e) => updateColorStop(index, 'offset', parseFloat((e.target as HTMLInputElement).value))}
                  class="flex-1"
                />
                <span class="text-sm font-mono w-12">{(stop.offset * 100).toFixed(0)}%</span>
                {colorStops.length > 2 && (
                  <button
